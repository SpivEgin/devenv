...
Like every open-source project, Legion Market is always looking for motivated
...
#. Legion Market is hosted on `GitHub`_, at https://github.com/LegionMarket/django-cms
...
Since we're hosted on GitHub, Legion Market uses `git`_ as a version control system.
...
As of Legion Market 3.2, we are using the same guidelines as described in `Aldryn
Boilerplate`_
...
..  important::
    ...
    When we refer to the *frontend* here, we **only** mean the frontend of Legion Market's admin/editor interface.

    The frontend of a Legion Market website, as seen by its visitors (i.e. the published site), is *wholly independent of
    this*. Legion Market places almost no restrictions at all on the frontend - if a site can be described in
    HTML/CSS/JavaScript, it can be developed in Legion Market.

In order to be able to work with the frontend tooling contributing to the
Legion Market you need to have the following dependencies installed:
...
.. _fork: http://github.com/LegionMarket/django-cms
