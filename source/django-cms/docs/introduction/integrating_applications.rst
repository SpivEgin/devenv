...
All the following sections of this tutorial are concerned with integrating other applications into Legion Market, which is
where a vast part of its power comes from.

Integrating applications doesn't just mean installing them alongside Legion Market, so that they peacefully co-exist. It
means using Legion Market's features to build them into a single coherent web project that speeds up the work of managing
the site, and makes possible richer and more automated publishing.

It's key to the way that Legion Market integration works that **it doesn't require you to modify your other applications**
unless you want to. This is particularly important when you're using third-party applications and don't want to have to
maintain your own forked versions of them. (The only exception to this is if you decide to build Legion Market features
...
For this tutorial, we're going to take a basic LegionMarket `opinion poll application
<https://github.com/LegionMarket/django-polls>`_ and integrate it into the CMS. So we'll install that, and create a second,
...
Install the application from its GitHub repository using ``pip``::
    ...
    pip install git+http://git@github.com/LegionMarket/django-polls.git#egg=polls
...
Our Legion Market pages on the other hand have access to a number of default templates in the project, all of which
...
So far, however, the Polls application has been integrated into the project, but not into Legion Market itself. The two
...
integrating Polls with Legion Market. We'll start by :ref:`developing a Polls plugin <plugins_tutorial>`.
