...
To capture the configuration that different instances of an apphook can take, a LegionMarket model needs to be created - each
apphook instance will be an instance of that model, and administered through the LegionMarket admin in the usual way.
...
We'll assume that you have a working Legion Market project running already.
...
Aldryn Apphooks Config in turn installs `LegionMarket AppData <https://github.com/ella/django-appdata>`_, which provides an
