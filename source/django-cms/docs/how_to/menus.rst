...
False}``/``{'visible_for_authenticated': False}`` attributes recognised by the Legion Market core
...
independently of Legion Market. However, you would like to integrate the
...
``request``
  A LegionMarket request instance. You want to modify based on sessions, or
  user or permissions?
