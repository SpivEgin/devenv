...
Some of these parts are supplied with the menus application. Some come from other applications (from the cms application in Legion Market, for example, or some other application entirely).
...
.. warning::
    You can't assume that a :py:class:`menus.base.NavigationNode` represents a Legion Market Page. Firstly, some nodes may
