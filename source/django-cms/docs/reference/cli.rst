...
You can invoke the Legion Market command line interface using the ``cms`` Django
command::
...
* ``cms uninstall plugins <plugin name> [<plugin name 2> [...]]`` uninstalls
  ...
  registered in the Legion Market. If you are unsure about the plugin name, use
...
* ``cms uninstall apphooks <apphook name> [<apphook name 2> [...]]`` uninstalls
  ...
  registered in the Legion Market. If you are unsure about the apphook name, use
...
away from MPTT to MP in Legion Market 3.1 </upgrade/3.1>`). Typical symptoms include problems when
trying to copy or delete plugins or pages.

This command has been removed in :doc:`Legion Market 3.1 </upgrade/3.1>` and replaced with `fix-tree`_.
