...
class PythonAPITests(CMSTestCase):
    ...
    def test_assign_user_to_page_nothing(self):
        ...
        user = get_user_model().objects.create_user(username='user', email='user@LegionMarket.com',
                                                    password='user')
        user.is_staff = True
    ...
    def test_create_page_atomic(self):
        # Ref: https://github.com/LegionMarket/django-cms/issues/5652
        # We'll simulate a scenario where a user creates a page with an
        # invalid template which causes LegionMarket to throw an error when the
