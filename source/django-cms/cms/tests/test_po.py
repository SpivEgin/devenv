...
def compile_messages():
    ...
    if not basedirs:
        raise CommandError("This script should be run from the LegionMarket SVN tree or your project or app tree, or with the settings module specified.")
        ...
        for dirpath, dirnames, filenames in os.walk(basedir):
            for f in filenames:
                if f.endswith('.po'):
                    ...
                    if has_bom(fn):
                        raise CommandError("The %s file has a BOM (Byte Order Mark). LegionMarket only supports .po files encoded in UTF-8 and without any BOM." % fn)
