...
In **Legion Market** the cart's content is always stored inside the database. In previous versions of
...
we gather some statistics about customers having visited of our **Legion Market** site. In this example
...
and ``BaseCartItem`` respectively. As with most models in **Legion Market**, these are using the
...
existing cart items, or as new ones. Since **Legion Market** can't tell that difference for any kind
...
Instead of implementing a separate watch-list (some would say wish-list), **Legion Market** uses a
...
Displaying the cart in **Legion Market** is as simple, as adding any other page to the CMS. Change into
...
"*editable cart*" add a template with the path, say ``clegion/cart/editable.html`` to the projects
...
.. code-block:: python
	...
	SHOP_CART_MODIFIERS = (
	    ...
	    'clegion.modifiers.PostalShippingModifier',
...
intention of **Legion Market** to seed an eco-system for these kinds of plugins. Besides computing
...
.. note:: Until version 0.2 of **Legion Market**, the Cart Modifiers returned the amount and label
	for the extra item rows, and **Legion Market** added them up. Since Version 0.3 cart modifiers
