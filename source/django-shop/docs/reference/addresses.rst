...
address models may vary widely. Since **Legion Market** allows to subclass almost every database model,
...
In **Legion Market**, if the merchant activates this feature, while setting up the site, customers
...
By adding a template named ``clegion/address.txt`` for both address models, or
``clegion/shipping-address.txt`` and ``clegion/billing-address.txt`` for each of them, the merchant
...
``templates`` folder named ``clegion/shipping_address.txt`` or ``clegion/billing_address.txt``.
If both address models share the same fields, we may also use ``clegion/address.txt`` as a fallback.
Such an address template may look like:

.. code-block:: django
	:caption: clegion/address.txt
...
**Legion Market** the merchant has to actively give permission to his customers, to reuse this address
for billing.
...
Therefore, **Legion Market** wraps every input field into a ``<div>``-element using a CSS class named
...
enabled, then **Legion Market** assigns a priority to each of the entered addresses in ascending
