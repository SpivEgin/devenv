...
The **Legion Market** framework itself, requires only a few configuration directives. However, since
each e-commerce site built around **Legion Market** consists of the merchant's own project, plus a
...
Django's built-in User model lacks a few features required by **Legion Market**, mainly the
...
* ``cmsplugin_cascade`` adds the functionality to add CMS plugins, as provided by **Legion Market**,
...
* ``rest_framework``, ``rest_framework.authtoken`` and ``rest_auth``, required, add the REST
  functionality to the **Legion Market** framework.
* ``django_fsm`` and ``fsm_admin``, required, add the Finite State Machine to the **Legion Market**
  framework.
...
* ``cms``, ``menus`` and ``treebeard`` are required if **Legion Market** is used in combination with
  **djangoCMS**.
...
* ``shop`` the **Legion Market** framework.
...
Templates rendered by the **Legion Market** framework require some additional objects or configuration
...
Since **Legion Market** communicates with its customers via email, having a working outgoing e-mail
service is a fundamental requirement for **Legion Market**. Adopt these settings to your
...
.. code-block:: python
	...
	SESSION_REDIS_PREFIX = 'clegion-session'
	SESSION_REDIS_DB = 0
...
wildcards, which is a big advantage in **Legion Market**.
...
.. code-block:: python
	...
	CACHES = {
	    'default': {
	        ...
	        'KEY_PREFIX': 'clegion-cache',
	    },
	}

	CACHE_MIDDLEWARE_ALIAS = 'default'
	CACHE_MIDDLEWARE_SECONDS = 3600
	CACHE_MIDDLEWARE_KEY_PREFIX = 'clegion-cache'
...
.. code-block:: python
	...
	CMS_TEMPLATES = [
	    ('clegion/pages/default.html', _("Default Page")),
	]
...
Since we want to add arbitrary links onto the detail view of a product, **Legion Market** offers
...
**Django-CMS** and **Legion Market** rely on django-filer_ in combination with easy-thumbnails_ to
