...
While Django doesn't impose any client side framework, **Legion Market** has to. Here we have to
...
Therefore the authors of **Legion Market** have decided to add reusable JavaScript components.
Here the most obvious choice would have been jQuery, since it is used by the Django administration
backend. However by using jQuery, web designers adopting templates for their **Legion Market**
...
directives_ are provided by the **Legion Market** framework. Frontend developers therefore do not
have to add or adopt any JavaScript code, except for the initialization.

.. note:: Since **Legion Market** uses REST for every part of the communication, the client side
...
.. code-block:: django
	:caption: clegion/pages/base.html
	...
	angular.module('pFlyers', ['ngAnimate', 'ngMessages', 'ngSanitize',
...
The **Legion Market** framework declares a bunch of Angular directives and controllers, grouped into
...
.. code-block:: Django
	...
	angular.module('pFlyers', [/* other dependencies */
	    {% render_block "ng-requires" postprocessor "djng.sekizai_processors.module_list" %}
	])
...
.. code-block:: Django
	...
	angular.module('pFlyers', [/* module dependencies */]
	).{% render_block "ng-config" postprocessor "djng.sekizai_processors.module_config" %};
