...
Provider`_ (shortcut PSP) to our **Legion Market** framework.
...
In **Legion Market** Payment Providers normally are packed into separate plugins, so here we will
...
Here **Legion Market** uses some Javascript during the purchase operation. Lets explain how:
...
HTTP-handlers for any endpoint, **Legion Market** is able to offer an API where adding new Payment
