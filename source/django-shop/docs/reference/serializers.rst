...
*Controllers* can sometimes be found on the server and sometimes on the client. In **Legion Market**
...
Moreover, since **Legion Market** uses **django-CMS** to organize all available components, a classic
...
As we already know, all database models from the **Legion Market** framework are owned by the merchant
...
Every URL which is part of part of **Legion Market**, namely the product's list and detail views, the
...
example :class:`clegion.serializers.ProductSummarySerializer`. This has been customized to represent
...
:class:`clegion.serializers.ProductDetailSerializer` which has been customized to represent our
...
This apphook is shipped with **Legion Market** itself and can be found at ``shop/cms_apps.py``.

As with all other Views used by **Legion Market**, the content of this View can also be rendered in
its dictionary structure, instead of HTML. Just append ``?format=api`` to the URL and get the Order
details. In our clegion example this may look like:
...
the expected results. In our clegion example this may look like:
...
In previous versions of **Legion Market**, these kinds of controller implementations had to be
...
or some kind of Ajax. Now **Legion Market** is shipped with reusable View classes, and the merchant's
