...
In **Legion Market**, we model each product according to its own properties, the color for instance.
...
In **Legion Market** showing a list of products, normally is controlled by the classes
...
.. code-block:: python
	:caption: clegion/urls/catalog.py
...
.. code-block:: python
	:caption: clegion/filters.py

	from django.forms import forms, widgets
	import django_filters
	from djng.forms import NgModelFormMixin
	from clegion.models.product import MyProduct, Manufacturer
...
We then can add this filter class to our product list view. In **Legion Market** this normally is done
...
Since **Legion Market** honours the principle of cohesion, each filter set is responsible for providing
...
**Legion Market** is shipped with a reusable directive named ``shopProductFilter``.
...
.. code-block:: javascript
	...
	angular.module('pFlyers', [..., 'django.shop.filter', ...]);
