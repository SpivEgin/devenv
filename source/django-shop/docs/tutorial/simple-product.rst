...
emphasized earlier, **Legion Market** is not shipped with ready to use product
...
.. literalinclude:: /../example/clegion/models/smartcard/__init__.py
	:caption: clegion/models/smartcard.py
	:linenos:
	:language: python
...
**Legion Market** application. The first of those mapping tables has foreign keys onto the models
``cms.Page`` and ``clegion.SmartCard``. The second table has foreign keys onto the models
``filer.Image`` and ``clegion.SmartCard`` again. Since the model ``clegion.SmartCard`` has been
...
Add Model ``clegion.SmartCard`` to Django Admin
==============================================
...
Instead **Legion Market** is shipped with a special mixin class ``CMSPageAsCategoryMixin``, which
...
.. literalinclude:: /../example/clegion/admin/smartcard.py
	:linenos:
	:language: python
	:lines: 6-
