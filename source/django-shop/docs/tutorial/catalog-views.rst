...
In **Legion Market** the response views do not distinguish whether the product's information shall
...
we need a detail view to render each product type. The **Legion Market** framework supplies two
such serializers:
...
The **Legion Market** framework does not know which of those fields have to be serialized, therefore
it requires some help from the programmer:

.. code-block:: python
    :caption: clegion/product_serializers.py
    :linenos:

    from shop.serializers.bases import BaseProductSummarySerializer
    from clegion.models.polymorphic.product import Product
...
.. code-block:: django
    :caption: clegion/products/catalog-smartcard-media.html
...
been moved into a specialized serializer. This is because **Legion Market** can not presuppose that
...
For this purpose **Legion Market** is shipped with an ``AddToCartSerializer``. It can be overridden
