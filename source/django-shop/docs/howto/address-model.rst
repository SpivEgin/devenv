...
their selling region. Since **Legion Market** allows to override each model, we simply might want to
write our own one.
...
In **Legion Market** each address model has a foreign key onto the customer model, hence all of the
